...
export default function NovoAgendamentoForm() {
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        if (selectedDates.length === 0)
            return alert('Por favor, selecione pelo menos uma data');
        if (!form.professor_id || !form.disciplina_id)
            return alert('Por favor, selecione o professor e a disciplina');
        if (!form.laboratorio_id)
            return alert('Por favor, selecione um laboratório');
        if (!form.quantidade_alunos || parseInt(form.quantidade_alunos) <= 0)
            return alert('Por favor, informe a quantidade de alunos');
        try {
